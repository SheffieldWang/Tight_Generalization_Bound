...
from train_utils import ClassificationTrainer,hinge_loss_fn
...
class ExperimentManager:
    ...
    def run_experiments(self):
        ...
        for i in range(self.n_repeats):
            ...
            _= trainer.train()
            train_loss,train_accuracy = trainer.get_train_metrics()
            test_loss,test_accuracy = trainer.get_test_metrics()
            self.metrics.update("loss", train_loss, split="train", index_type="repeat")
            self.metrics.update("accuracy", train_accuracy, split="train", index_type="repeat")
            self.metrics.update("loss", test_loss, split="test", index_type="repeat")
            self.metrics.update("accuracy", test_accuracy, split="test", index_type="repeat")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--alpha', type=float, default=0.25,
                        help='Fraction of data to shuffle (default: 0.25)')
    ...
    config = {
        ...
        'optimizer': optimizer,
        'seed':seed,
        'project_name': f'Random_Label',
        'group_name': f'alpha_{alpha}_n_samples_{n_samples}_seed_{seed}'
    }
    ...
    # Randomly assign labels {-1, 1} to training data using JAX
    ...
    random_train_labels = jax.random.choice(
        subkey, 
        jnp.array([-1, 1]), 
        shape=(n_train_samples,)
    )
    train_loader.dataset.targets = random_train_labels
    
    # For test data
    n_test_samples = len(test_loader.dataset)
    key, subkey = jax.random.split(key)
    random_test_labels = jax.random.choice(
        subkey, 
        jnp.array([-1, 1]), 
        shape=(n_test_samples,)
    )
    test_loader.dataset.targets = random_test_labels
    
    print(f"Randomly assigned labels {-1, 1} to {n_train_samples} training samples and {n_test_samples} test samples using JAX")
